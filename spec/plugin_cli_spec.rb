...
RSpec.describe "Mini CI plugin CLI" do
  ...
  it "validates explicit plugins without executing a pipeline" do
    exit_code, stdout, stderr = run_cli(["plugins", "validate", "--plugin", "examples/plugins/run_logger.rb", "--plugin", "examples/plugins/message_item.rb"])
  ...
  it "runs a pipeline with a plugin-defined item" do
    exit_code, stdout, stderr = run_cli(["run", "examples/plugin-basic-pipeline.yml", "--plugin", "examples/plugins/message_item.rb"])
  ...
  it "loads validators during validate but does not run runtime callbacks" do
    ...
    exit_code, stdout, stderr = run_cli(["validate", "examples/plugin-basic-pipeline.yml", "--plugin", "examples/plugins/run_logger.rb", "--plugin", "examples/plugins/message_item.rb"])
  ...
  it "returns a configuration error for plugin validator failures" do
    exit_code, stdout, stderr = run_cli(["validate", "examples/plugin-validation-failure-pipeline.yml", "--plugin", "examples/plugins/policy_validator.rb"])
  ...
  it "returns non-zero for callback failures after preserving command output" do
    exit_code, stdout, stderr = run_cli(["run", "examples/plugin-callback-failure-pipeline.yml", "--plugin", "examples/plugins/callback_failure.rb"])
  ...
  it "keeps command failures primary when after_run also fails" do
    exit_code, stdout, stderr = run_cli(["run", "examples/failing-pipeline.yml", "--plugin", "examples/plugins/callback_failure.rb"])
